...
void pipeline(char ***process, int n){	
	...
	for(i = 0; i < n - 1; i++){
		...
		if((pid = fork()) == 0){ // child
			...
			fprintf(stderr, "error: exec doesn't work; pr = %d\n", i + 1);
			exit(2);
		} 
		else if(pid == -1){
			perror("Error: fork doesn't work\n");
			exit(1);
		}
		else{ // parrent
			close(f0);
	...
	//	wait(&status);
		if(!WIFEXITED(status)) fprintf(stderr, "error: status = %d\n", status);
		else if(WEXITSTATUS(status)) fprintf(stderr, "exit_child = %d\n", status);
	}
}
